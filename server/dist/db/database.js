...
exports.addRescueLog = addRescueLog;
exports.recordVerificationEvent = recordVerificationEvent;
exports.recordDeliveryEvidence = recordDeliveryEvidence;
exports.calculateIntegrityScore = calculateIntegrityScore;
const better_sqlite3_1 = __importDefault(require("better-sqlite3"));
const path_1 = __importDefault(require("path"));
const dbPath = path_1.default.resolve(__dirname, '../../food_rescue.db');
...
function initDatabase() {
  ...
  `);
    ...
    try {
        db.exec(`ALTER TABLE food_packages ADD COLUMN seal_code TEXT`);
    }
    catch (e) { }
    try {
        db.exec(`ALTER TABLE food_packages ADD COLUMN donor_photo_url TEXT`);
    }
    catch (e) { }
    try {
        db.exec(`ALTER TABLE food_packages ADD COLUMN pickup_photo_url TEXT`);
    }
    catch (e) { }
    try {
        db.exec(`ALTER TABLE food_packages ADD COLUMN delivery_photo_url TEXT`);
    }
    catch (e) { }
}
function addNotification(userId, type, title, message, donationId) {
...
function addRescueLog(donationId, actorName, actorRole, action, status, details) {
...
function recordVerificationEvent(donationId, packageId, eventType, userId, role, result, metadata, location) {
  ...
  `).run(id, donationId, packageId, eventType, userId, role, location || null, result, metaStr);
    addRescueLog(donationId, `User ${userId.substring(0, 8)}`, role, eventType, result, metaStr || undefined);
}
function recordDeliveryEvidence(donationId, packageId, evidenceType, imageUrl, capturedBy, capturedRole, sealCode, notes) {
...
function calculateIntegrityScore(donationId) {
    const pkgs = db.prepare('SELECT * FROM food_packages WHERE donation_id = ?').all(donationId);
    const evs = db.prepare('SELECT * FROM verification_events WHERE donation_id = ?').all(donationId);
    const evd = db.prepare('SELECT * FROM delivery_evidence WHERE donation_id = ?').all(donationId);
    const disputes = db.prepare('SELECT * FROM delivery_disputes WHERE donation_id = ? AND status != "RESOLVED"').all(donationId);
    const routeDevs = db.prepare('SELECT * FROM driver_locations WHERE donation_id = ? AND is_off_route = 1').all(donationId);
    let score = 100;
    const reasons = [];
