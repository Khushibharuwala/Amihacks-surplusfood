...
export interface MatchingWeights {
  needWeight: number; // 0.25
  urgencyWeight: number; // 0.30
  distanceWeight: number; // 0.20
  capacityWeight: number; // 0.10
  driverWeight: number; // 0.15
}
...
export function evaluateAndMatchDonation(
  ...
  for (const ngo of ngos) {
    ...
    const typeMatch =
      ...
      acceptedTypes.includes('ALL') ||
      acceptedTypes.some(
        (t) => t.toLowerCase() === donation.food_type.toLowerCase()
      );
    ...
    if (!typeMatch) {
      evaluations.push({
        ...
        organizationName: ngo.organization_name,
        eligible: false,
        rejectionReason: `Food type incompatible: NGO accepts [${acceptedTypes.join(
          ', '
        )}], donation is ${donation.food_type}`,
        distanceKm,
        availableCapacityKg: availableCapacity,
      });
      continue;
    ...
    const feasibleDrivers = drivers.filter((driver) => {
      ...
      if (driver.vehicle_capacity_kg < donation.quantity_kg) return false;
      return true;
    });
